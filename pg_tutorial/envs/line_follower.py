...
class LineFollowerConstantSpeedEnv(LineFollowerEnv):
    """A variant of LineFollowerEnv that uses constant forward speed with steering control.

    Instead of commanding [left_wheel_speed, right_wheel_speed], the agent controls
    a steering angle which maps to angular velocity. The forward speed is held constant.

    The action space is a single value in [-1, 1] representing steering:
    - -1 = full left turn
    - 0 = straight
    - 1 = full right turn
    """
    ...
    def __init__(
        ...
        screen_width: int = 1000,
        screen_height: int = 800,
        constant_speed: float = 100.0,
    ) -> None:
        """Initialize the constant speed environment.

        Args:
            constant_speed: The constant forward speed to maintain (in wheel speed units).
                           This determines the base speed of the robot.
        """
        super().__init__(
        ...
        # Update action space: single steering value in [-1, 1]
        # Steering maps to angular velocity via: angular_velocity = steering * max_angular_velocity
        self.action_space = spaces.Box(
            low=-1.0,
            high=1.0,
            shape=(1,),
        ...
        # Update observation space to match new action space structure
        # Observation: [lateral_error, heading_error, forward_velocity,
        #               angular_velocity, left_wheel_speed, right_wheel_speed,
        #               curvature, lookahead_lat_2, lookahead_lat_4, lookahead_lat_6]
        obs_high = np.array(
            [
                ...
                self.max_wheel_speed * self.wheel_radius * 2.0,  # fwd vel
                self.max_wheel_speed * self.wheel_radius * 2.0 / self.wheel_base,  # ang vel
                self.max_wheel_speed,  # left wheel (for consistency)
                self.max_wheel_speed,  # right wheel (for consistency)
                np.pi / 10.0,  # curvature (radians per unit)
                self.off_track_threshold,  # lookahead lat 2
                self.off_track_threshold,  # lookahead lat 4
                self.off_track_threshold,  # lookahead lat 6
    ...
    def _apply_action(self, action: NDArray[np.float32]) -> tuple[float, float]:
        """Decode steering action and apply constant forward speed.

        The action is a single steering value in [-1, 1] that determines
        the angular velocity while maintaining constant forward speed.

        Parameters
        ----------
        action:
            Steering command in [-1, 1], where -1 is full left, 0 is straight, 1 is full right.

        Returns
        -------
        forward_velocity:
            Constant forward speed scaled by wheel radius.
        angular_velocity:
            Steering-induced angular velocity based on action.
        """
        # Clip and decode steering action
        steering = float(np.clip(action[0], -1.0, 1.0))

        # Add noise to steering if configured
        if self.action_noise_std > 0.0:
            noise = self.np_random.normal(0.0, self.action_noise_std, size=1)
            steering += float(noise[0])

        # Compute angular velocity from steering
        # Maximum angular velocity when steering is at full lock
        # This is similar to a car turning at constant speed: omega = v * tan(delta) / L
        # For simplicity, we use a linear mapping: omega = steering * max_angular_velocity
        max_angular_velocity = self.max_wheel_speed / self.wheel_base  # from differential drive limits
        angular_velocity = steering * max_angular_velocity

        # Constant forward speed (scaled by wheel radius to get linear velocity)
        forward_velocity = self.constant_speed * self.wheel_radius

        # Store wheel speeds for observation (derived from forward and angular velocity)
        # left_linear = forward - (wheel_base/2) * angular_velocity
        # right_linear = forward + (wheel_base/2) * angular_velocity
        # Then convert to wheel speeds: wheel_speed = linear_velocity / wheel_radius
        left_linear = forward_velocity - (self.wheel_base / 2.0) * angular_velocity
        right_linear = forward_velocity + (self.wheel_base / 2.0) * angular_velocity
        self.left_wheel_speed = left_linear / self.wheel_radius
        self.right_wheel_speed = right_linear / self.wheel_radius
